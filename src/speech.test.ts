...
describe('speech function', () => {
  ...
  it('When providing options to speech function, it configures recognition with those options', () => {
    ...
    expect(mockRecognitionInstance.addEventListener).toHaveBeenCalledWith('result', expect.any(Function));
    expect(mockRecognitionInstance.addEventListener).toHaveBeenCalledWith('start', options.onStart);
    expect(mockRecognitionInstance.addEventListener).toHaveBeenCalledWith('end', options.onEnd);
    expect(mockRecognitionInstance.addEventListener).toHaveBeenCalledWith('error', expect.any(Function));
  });
  ...
  it('When calling dispose method, it aborts recognition and removes event listeners', () => {
    ...
    // Assert: 期待される結果を確認
    expect(mockRecognitionInstance.abort).toHaveBeenCalledTimes(1);
    expect(mockRecognitionInstance.removeEventListener).toHaveBeenCalledTimes(4);
  });
...
describe('Browser compatibility', () => {
  ...
  it('When browser does not support SpeechRecognition API, it throws an error', () => {
    ...
    expect(() => speech({})).toThrow('Speech Recognition is not supported in this browser');
  });
});